...
- `/vault/secrets/*` for S3 credentials
- `/etc/bit/trusted-certs/*` for the mounted trusted CA material
...
```text
  ...
  | S3_SECRET_ACCESS_KEY=...  |    | /etc/bit/trusted-certs/*     |
  +---------------------------+    +------------------------------+
```
...
```bash
docker build -f bdw/Dockerfile -t bit-data-workbench:0.3.18 .
```
...
```text
docker-hub.nexus.bit.admin.ch/svabra/bit-data-workbench:0.3.18
```
...
3. Have Vault inject the S3 credentials into `/vault/secrets`:
   `s3-access-key-id` and `s3-secret-access-key`.
4. Mount the namespace ConfigMap `bit-ros-trusted-certs` into the pod at `/etc/bit/trusted-certs`.
5. Keep the endpoint, bucket, and TLS mode in `k8s/duckdb-configmap.yaml`.
6. Do not place S3 credentials in `k8s/duckdb-secret.example.yaml`.
...
The app now auto-builds a CA bundle from the mounted files in `/etc/bit/trusted-certs`, so `S3_CA_CERT_FILE` can stay unset unless you want to force a specific file path.
...
```text
    ...
    +-----------------------------------------------+
                   ...
                   v
    +-----------------------------------------------+
    | Pod mount: /etc/bit/trusted-certs             |
    +-----------------------------------------------+
```
