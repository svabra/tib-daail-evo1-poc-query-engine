...
class PostgresSmokeDataGenerator(DataGenerator):
    ...
    title = "PostgreSQL OLAP Smoke Loader"
    description = (
        "Generates a semantic sales-order style dataset and writes it into PostgreSQL OLAP "
    ...
    default_target_name = "generated_orders_pg"
    tags = ("postgres", "orders", "smoke")
    ...
    def run(self, context: DataGeneratorContext) -> DataGeneratorResult:
        ...
        try:
            context.report(
                ...
                target_relation=f"pg_olap.public.{table_name}",
            )
            ...
            for batch_index, start_row in enumerate(range(0, total_rows, batch_rows), start=1):
                ...
                context.report(
                    ...
                    message=f"Inserted {inserted_rows:,} rows into pg_olap.public.{table_name}.",
                    target_name=table_name,
                    target_relation=f"pg_olap.public.{table_name}",
            ...
            return DataGeneratorResult(
                ...
                generated_rows=inserted_rows,
                generated_size_gb=approximate_size_gb(inserted_rows, self.approximate_row_bytes),
                message=f"Generated {inserted_rows:,} rows in pg_olap.public.{table_name}.",
            )
