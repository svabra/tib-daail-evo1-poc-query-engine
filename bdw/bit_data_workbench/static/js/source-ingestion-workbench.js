...
function renderHubSources(root, context) {
  ...
  if (!sources.length) {
    target.innerHTML = '<div class="sourcing-empty"><strong>No approved Oracle source yet</strong><p>Request access from the source owner to start an ingestion.</p><a href="/ingestion-workbench/sourcing/request">Request a new Data Source</a></div>';
    return;
  }
  target.innerHTML = sources.map((source) => {
    const relations = Array.isArray(source.relations) ? source.relations : [];
    const firstRelation = relations[0] || {};
    return `<article class="source-ingestion-source-card">
      <div><span class="source-ingestion-source-kind">Oracle PoC &middot; ${escapeHtml(source.site || "BIT")}</span><h4>${escapeHtml(source.displayName || source.id)}</h4><p>${escapeHtml(source.databaseName || source.id)} &middot; Owner ${escapeHtml(source.owner || "")}</p></div>
      <dl><div><dt>Grant</dt><dd>Active</dd></div><div><dt>Objects</dt><dd>${relations.length}</dd></div></dl>
      <div class="source-ingestion-card-actions"><a class="modal-button" href="${relationQuery(source.id, firstRelation)}">Create ingestion</a><a href="/data-sources?source_id=${encodeURIComponent(source.id)}">Browse source</a></div>
    </article>`;
  }).join("");
}
...
function renderHubDefinitions(root, payload) {
  ...
  if (!definitions.length) {
    target.innerHTML = '<div class="sourcing-empty"><strong>No ingestion definitions yet</strong><p>Start with an approved Oracle relation. The same definition can run once or every hour.</p><a href="/ingestion-workbench/sourcing/ingestions/new">Create your first ingestion</a></div>';
    return;
  }
...
function defaultObjectKey(sourceId, schema, relation) {
  return `ingestions/oracle/${sourceId}/${schema.toLowerCase()}/${relation.toLowerCase()}.parquet`;
}
...
function updateWizardRelationOptions(root, context, preselectedRelation = "") {
  ...
  if (source) grantNote.textContent = `Active DaCa grant · ${source.displayName} · ${source.site || "BIT"} · checked again before every run`;
}
...
function updateWizardDestination(root, { force = false } = {}) {
  ...
  if (relation && (force || !keyInput.value)) keyInput.value = defaultObjectKey(sourceId, relation.schema, relation.name);
...
function validateWizardStep(root, step) {
  ...
  if (step === 1 && !selectedRelation(root)) {
    showAlert(root, "Choose one Oracle table or view.");
    return false;
  }
  showAlert(root);
  return true;
...
async function initializeWizard(root) {
  ...
  try {
    ...
    if (!root.isConnected) return;
    const sourceSelect = root.querySelector("[data-source-ingestion-source]");
    sourceSelect.replaceChildren(new Option("Choose an approved Oracle source", ""));
    (context.sources || []).forEach((source) => sourceSelect.append(new Option(`${source.displayName} · ${source.databaseName}`, source.id)));
    ...
    (context.visibleBuckets || []).forEach((bucket) => bucketOptions?.append(new Option(bucket, bucket)));
    ...
    root.querySelector("[data-source-ingestion-bucket]").addEventListener("input", () => updateWizardDestination(root));
