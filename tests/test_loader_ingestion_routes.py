...
class LoaderAndIngestionRouteTests(unittest.TestCase):
    ...
    def test_ingestion_workbench_partial_renders_csv_entry_surface(self) -> None:
        ...
        self.assertIn('data-csv-parquet-optimization-panel', body)
        self.assertIn('data-csv-parquet-optimization-mode', body)
        self.assertIn('data-csv-hive-partitioning', body)
        self.assertIn('data-csv-optimization-column-basis', body)
        self.assertIn('data-csv-hive-partitioning-explanation', body)
        self.assertIn('data-csv-partition-column-options', body)
        self.assertIn('data-csv-sort-column-options', body)
        self.assertIn('data-csv-index-column-options', body)
        self.assertIn("DuckDB remains the query engine for Shared Workspace files.", body)
        self.assertIn("system records a request for conservative Parquet layout choices", body)
        self.assertIn("year=2026/month=05", body)
        self.assertIn("local DuckDB cache indexes", body)
        self.assertIn("too many partitions creates many small files and can slow queries", body)
        ...
        self.assertIn('data-file-parquet-optimization-panel', body)
        self.assertIn('data-file-parquet-optimization-mode', body)
        self.assertIn('data-file-hive-partitioning', body)
        self.assertIn('data-file-optimization-column-basis', body)
        self.assertIn('data-file-hive-partitioning-explanation', body)
        self.assertIn('data-file-partition-column-options', body)
        self.assertIn('data-file-sort-column-options', body)
        self.assertIn('data-file-index-column-options', body)
