...
class NotebookEditorUiRegressionTests(unittest.TestCase):
    ...
    def test_notebook_stage_pipeline_has_mode_graph_table_and_actions(self) -> None:
        ...
        self.assertIn('data-pipeline-tooltip="Run all pipeline stages in dependency order"', markup_source)
        self.assertIn('data-pipeline-tooltip="Run all pipeline stages in dependency order"', workspace_template)
        ...
        self.assertIn("animation: pipeline-node-computing-glow 2.8s ease-in-out infinite;", css_source)
        ...
        self.assertIn("min-width: 320px;", css_source)
